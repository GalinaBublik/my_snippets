...
(function() {
    ...
    for(var i in post_types){
        rs_val[i] = {
            text: post_types[i]
            /*, onclick : function() {
            tinymce.execCommand('mceInsertContent', false, '[list type="' + post_types[i] + '" posts="-1" orderby="date" order="DESC"]' );
            }*/
        };
    }

    tinymce.create('tinymce.plugins.postList', {
        init : function(editor, url) {

            editor.addButton('list', {
                title : 'Add Post type block',
                image : url+'/images/icon_97.png',
                onclick : function() {
                    eval('create_1pls1_list(nexalist, editor);open_dialogue_list("#'+nexalist.id+'")');
                }
            });
        },
    ...
    // Register plugin
    tinymce.PluginManager.add('list', tinymce.plugins.postList);
})();

/*--------Open/close dialog by click on button-------*/

function open_dialogue_list(dialogueid,width,height){
    console.log(dialogueid);
    if(typeof(width)==='undefined') width = 'auto';
    if(typeof(height)==='undefined') height = 'auto';
    jQuery( dialogueid ).dialog({
        dialogClass : 'wp-dialog list-dialog',
        autoOpen: true,
        height: height,
        width: width,
        modal: true
...
function create_1pls1_list(pluginObj, editor){
    //console.log(editor);
    ...
    var form = jQuery('<div id="'+pluginObj.id+'" class="1pls1-container" title="'+pluginObj.title+'">\
        ...
        <tr><th><label for="1pls1-number">Count:</label></th><td><input type="number" name="number" min="-1" id="1pls1-count-list" value="-1"/><br /><small>-1 - it\'s all<small></td></tr>\
        <tr><th><label for="1pls1-number">Column:</label></th><td><input type="number" name="number" min="1" max="4" id="1pls1-column-list" value="4"/><br /></td></tr>\
        <tr>\
            <td><input id="pagenavi" type="checkbox" name="pagenavi" value="1"/><label for="pagenavi">Show a pagenavi</label></td>\
        </tr>\
        </table>\
        <p class="submit"><input type="button" id="1pls1-list" class="button-primary" value="Insert Posts" name="submit" /></p></div>');
    ...
    form.find('#1pls1-list').click(function(){
        var shortcode = '[list ';
            
        shortcode += ' type="'+table.find('#1pls1-type').val()+ '" ';
        shortcode += ' column="'+table.find('#1pls1-column-list').val()+ '" ';
        shortcode += ' orderby="'+table.find('#1pls1-orderby').val()+ '" ';
        shortcode += ' order="'+table.find('#1pls1-order').val()+ '" ';
        shortcode += ' count="'+table.find('#1pls1-count-list').val()+ '" ';
        shortcode += ' pagenavi="'+table.find('#pagenavi:checked').length+ '" ';
